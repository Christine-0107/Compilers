...
protected:
    ...
    enum {BINARY, COND, UNCOND, RET, LOAD, STORE, CMP, ALLOCA, CALL, XOR, ZEXT, CAST, GEP};
};
...
public:
    ...
    void output() const;
    void genMachineCode(AsmBuilder*);
    enum {ADD, SUB, MUL, DIV, FADD, FSUB, FMUL, FDIV, MOD, AND, OR};
};
...
{
public:
    CastInstruction(unsigned opcode, Operand *dst, Operand *src, BasicBlock *insert_bb = nullptr);
    ~CastInstruction();
    void output() const;
    void genMachineCode(AsmBuilder*);
    enum {ITOF, FTOI};
...
{
public:
    GepInstruction(Operand *dst, Operand *base, std::vector<Operand *> offs, BasicBlock *insert_bb = nullptr);
    void output() const;
    void genMachineCode(AsmBuilder*);
};
