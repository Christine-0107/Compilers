...
public:
    ...
    void addFirstNode(ExprNode *node);
    void output(int level);
    void typeCheck();
    void genCode();
...
private:
    ArrayNode* arrayList;
public:
    Id(SymbolEntry *se) : ExprNode(se){dst = new Operand(se);/*SymbolEntry *temp = new TemporarySymbolEntry(se->getType(), SymbolTable::getLabel()); dst = new Operand(temp);*/};
    Type* getType();
    ...
    ArrayNode* getArrayList(){return this->arrayList;}
};
...
public:
    ...
    std::vector<ArrayDeclNode*> getArrayDeclList(){return arrayDeclList;}
};
...
private:
    Id *id;
    //ExprNode *expr;
    Node *expr; //数组为ArrayDeclNode, 其余为ExprNode
    bool isArray;
public:
    //VarDeclNode(Id *id,ExprNode *expr) : id(id),expr(expr){};
    VarDeclNode(Id *id, Node *expr, bool isArray) : id(id), expr(expr), isArray(isArray) {};
    void output(int level);
    void typeCheck();
    void genCode();
};
