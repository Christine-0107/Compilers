...
void FunctionDef::genCode()
{
    ...
    */
    ...
    {
        auto finalInstruction=(*block)->rbegin();
...
void BinaryExpr::typeCheck()
{
    ...
    if(!(type1->isInt()||type1->isConstInt()||type1->isFloat()||type1->isConstFloat())){
        fprintf(stderr, "type %s wrong in line xx", type1->toStr().c_str());
        exit(EXIT_FAILURE);
    }
    if(!(type2->isInt()||type2->isConstInt()||type2->isFloat()||type2->isConstFloat())){
        fprintf(stderr, "type %s wrong in line xx", type2->toStr().c_str());
        exit(EXIT_FAILURE);
    }
    // Int类型
    if((type1->isInt()&&type2->isInt()) || (type1->isConstInt()&&type2->isConstInt()) ||
        (type1->isInt()&&type2->isConstInt()) || (type1->isConstInt()&&type2->isInt())){
        symbolEntry->setType(TypeSystem::intType);
    }
    // Float类型
    else if(type1->isFloat() || type1->isConstFloat()){
        symbolEntry->setType(TypeSystem::floatType);
    }
    else if(type2->isFloat() || type2->isConstFloat()){
        symbolEntry->setType(TypeSystem::floatType);
    }
    // Error
    else{
